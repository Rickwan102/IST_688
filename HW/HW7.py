...
try:
    ...
    st.error("Please install the missing packages: pip install langchain-chroma langchain-openai")
    st.stop()
...
st.title("HW7 - News Reporting Bot for Global Law Firm")

if "OPENAI_API_KEY" not in st.secrets:
    st.error("Missing API key! Please add OPENAI_API_KEY in .streamlit/secrets.toml")
    st.stop()

# Initialize OpenAI client
openai_client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])

# Initialize Google Gemini client (only if API key exists)
google_client = None
...
def load_data():
    """
    Reads the CSV file containing news stories.
    Cached to avoid reloading each time the app runs.
    """
...
# Display error if the CSV file is not found
if df is None:
    st.error("CSV not found. Please double-check the file path.")
    st.stop()

st.success("News CSV file loaded successfully!")

# Allow user to preview the first few rows of the dataset
...
# Helper Function: Call LLM with or without function calling
...
def extract_function_call(response, vendor):
    ...
    if vendor == "OpenAI":
        ...
        # Gemini does not support function calling in the same way
